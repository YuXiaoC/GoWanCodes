...
window.FUSESDKAPI = {
  ...
  createRole ({token, data = {}, callback}) {
    ...
    let role_info = {
      ...
      platform: 'pptv',
      rolename: encodeURIComponent(data.role_name),
      sid: encodeURIComponent(data.server_id),
      time: Date.parse(new Date()) / 1000,
      userid: data.user_id
    }
    role_info['sign'] = makeSign(role_info, key)
    ajaxRequest('notify:createRole', role_info)
  },
  changeRole ({token, data = {}, callback}) {
    ...
    role_info['sign'] = makeSign(role_info, key)
    ajaxRequest('notify:changeRole', role_info)
  }
}

function ajaxRequest (namespace, data) {
  console.log('进入了ajaxRequest方法')
  Api.require(namespace, data, {
    methods: 'GET',
    dataType: 'jsonp',
    urlModel: 1,
    header: {
    ...
    domain: 'https://game.g.pptv.com/api/stat/notify_start_game.php'
  }).then((res) => {
