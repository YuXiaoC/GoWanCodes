...
let GD = window.GLOBAL_DATA  //全局数据
...
// 账号登录、手机登录、账号注册、手机注册、获取验证码、修改密码、
  ...
  if (action) {
    ...
    switch (action) {
      ...
      case "login-account":
        data = {
          mode: 0,
          account: GD.popboxInfo.account,     // 获取name:account的值
          password: GD.popboxInfo.password    // 获取name:password的值
        }
        if (data.account == "") {
        ...
        window.gw8SDK.login(data).then(res => {   //定义在gowansdk.js中   gw8SDK.login()
          if (res.code == 0) {
            FN.saveSession("USER_INFO", res.data)   //储存用户信息到Session stroage里面 ‘USER_INFO’
            loginSuccessCallback(res, data)       //登录成功执行传递res和请求data
          }
      ...
      case "register-account":
        ...
        actionStatus[action] = false
        window.gw8SDK.register(data).then(res => {  //提交注册
          if (res.code == 0) {
            showMsg("注册成功")
